...
class RocketRacerEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(low=0.0, high=np.inf, shape=(self.LIDARS,), dtype=np.float32)

        self.spaceship = SimpleNamespace(x=None, y=None, a=0.0, vx=0.0, vy=0.0, va=0.0)
    ...
    def step(self, action):
        ...
        # print(reward, self.stage_rewards)
        return None, reward, done, {}

    def reset(self):
        self.map = CircularMap(n=random.randint(8, 24), width=0.3)
        if self.viewer:
            self.viewer.close()
            self.viewer = None
        # self.viewer = None
        ...
        # self.spaceship.vy = 0.1*np.random.normal()
        ...
        # print(len(self.map.left) + len(self.map.right))
